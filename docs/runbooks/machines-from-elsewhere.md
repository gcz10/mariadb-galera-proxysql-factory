...
>   --cluster <KLASTER> --pool <PULA> --node <WEZEL> --gateway <BRAMA> \
>   --bridge <MOST> --storage <STORAGE> --image <STORAGE:OBRAZ> \
>   --nameserver "<DNS1> <DNS2>" [--role galera|restore]
> ```
> Poniższe kroki opisują działanie pod maską i służą do weryfikacji lub ręcznego wykonania.
>
> Parametry infrastruktury nie mają domyślnych wartości labowych. `--pool`
> ma pierwszeństwo przed `FLEET_POOL`, a `--node` przed `PROXMOX_VE_NODE`.
> Zamiast CIDR można podać pełny `--ip` i jawny `--prefix`; skrót ostatniego
> oktetu nie jest obsługiwany. Domyślne pozostają tylko zasoby VM i ścieżka klucza
> opisane przez `--help`. Brak wymaganych ustawień kończy pracę przed API.
...
: "${PROXMOX_VE_NODE:?Ustaw węzeł PVE}"; NODE="$PROXMOX_VE_NODE"
: "${FLEET_POOL:?Ustaw pulę tego wdrożenia}"; POOL="$FLEET_POOL"
: "${STORAGE:?Ustaw storage dysków i cloud-init}"
: "${IMAGE:?Ustaw identyfikator obrazu STORAGE:OBRAZ}"
: "${BRIDGE:?Ustaw most sieciowy}"
: "${IP_CIDR:?Ustaw pełny IPv4/PREFIX}"
: "${GATEWAY:?Ustaw bramę IPv4}"
: "${DNS_SERVERS:?Ustaw serwery DNS rozdzielone spacjami}"
...
for id in 9780 9781 9782; do
  ...
  used_vol=$(curl -sk -H "$H" "$EP/api2/json/nodes/$NODE/storage/$STORAGE/content" \
...
curl -sk -H "$H" -X POST "$EP/api2/json/nodes/$NODE/qemu" \
  ...
  --data-urlencode "nameserver=$DNS_SERVERS"
...
#
#    Pula ma TRZY stany, nie dwa. Wybrana pula to nasze. CUDZA pula to STOP.
...
members=$(curl -sk -H "$H" "$EP/api2/json/pools/$POOL" \
