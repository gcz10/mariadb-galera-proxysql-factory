...
# newclaude9-r9 — warstwa BAZODANOWA dziewiatego przebiegu budowy od zera.
#
...
locals {
  ...
  gateway    = "192.168.1.1"
  # VMID 9490-9493 wolne (sprawdzone przez API na pelnej liscie VM klastra PVE).
  # IP .168-.171 potwierdzone AKTYWNYM skanem sieci (ping + port 22/8006),
  ...
  vms = {
    n9g1 = { id = 9490, ip = 168, role = "galera",  cpu = 2, ram = 3072, disk = 40 }
    n9g2 = { id = 9491, ip = 169, role = "galera",  cpu = 2, ram = 3072, disk = 40 }
    n9g3 = { id = 9492, ip = 170, role = "galera",  cpu = 2, ram = 3072, disk = 40 }
    n9r1 = { id = 9493, ip = 171, role = "restore", cpu = 1, ram = 2560, disk = 40 }
  }
}
...
resource "proxmox_virtual_environment_vm" "node" {
  ...
  vm_id       = each.value.id
  description = "newclaude9-r9 Rocky 9 (${each.value.role}) — VMID ${each.value.id}"
  tags        = ["rocky9", "galera", "newclaude9", each.value.role, "n9"]
