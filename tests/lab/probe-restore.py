...
RESTORE_SCHEDULE = str(
    CLUSTER.get("backup", {}).get("restore_test_schedule", "0 4 * * 0")
)
...
def main():
    ...
    unixtime_raw = last_success.get("unixtime")
    age_days = None
    try:
        if unixtime_raw is None:
            failures.append("ISC-37 — missing or invalid last_success unixtime")
        else:
            unixtime = float(unixtime_raw)
            if unixtime <= 0 or not math.isfinite(unixtime):
                failures.append("ISC-37 — missing or invalid last_success unixtime")
            else:
    ...
    drill_age_str = f"drill {age_days:.2f}d ago" if age_days is not None else "drill age unknown"
    return finish(
        failures,
        undetermined,
        f"restore drill OK — {backup_name} restored to isolated host, "
        f"{rows_verified} rows verified, {drill_age_str} "
