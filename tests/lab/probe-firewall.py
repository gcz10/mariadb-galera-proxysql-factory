...
def report(failures: list[str], summary: str = "", undetermined: list[str] | None = None) -> int:
    """Report policy and command failures, including on early exits.

    Trzeci stan jest tu konieczny, bo „nie zmierzono" i „zmierzono, jest zle"
    maja przeciwne konsekwencje: pierwsze kaze naprawic droge pomiarowa, drugie
    polityke firewalla. Kod 2 jak w `_probe_common` (UNDETERMINED).
    """
