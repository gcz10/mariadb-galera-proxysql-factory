...
class PlatformPmmUpgradeSafetyContractTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        plays = yaml.safe_load(cls.text)
        cls.play_vars = plays[1]["vars"]
        cls.tasks = plays[1]["tasks"]
    ...
    def test_backup_runs_only_for_existing_container_with_changed_image(self):
        ...
        def upgrade_required(current, desired):
        ...
        self.assertFalse(upgrade_required("", desired), "swiezy deploy nie ma backupu")
        self.assertFalse(upgrade_required(desired, desired), "ten sam obraz nie ma backupu")
        self.assertTrue(
            upgrade_required("percona/pmm-server:3.9.0@sha256:old", desired),
            "istniejacy inny obraz wymaga backupu",
        )
    ...
    def test_backup_volume_is_unique_for_every_recreate_attempt(self):
        volume = str(self.play_vars["infra_pmm_backup_volume"])
        self.assertIn("lock.pmm.version", volume)
        self.assertIn(
            "ansible_date_time.epoch",
            volume,
            "retry po nieudanym Compose musi zrobic swiezy backup aktualnego /srv",
        )
