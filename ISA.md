...
- LUKA DOWODOWA — ISC-1 i ISC-2 są zaznaczone, ale nie mają stuba, co narusza `MASTER_PROMPT.md:940` („każdy checkbox ma deterministyczny probe i Verification stub"). ISC-1 (deploy na CZYSTYCH hostach) jest z natury niepowtarzalny na działającej flocie — zamknie go dopiero następna budowa klastra od zera. ISC-2 (drugi converge = `changed=0`) wymaga PRAWDZIWEGO przebiegu `site.yml`; trybu `--check` NIE da się użyć jako substytutu — zmierzone 2026-08-14 na `finalclaude-r9`: brama zdrowia Galery pada w check mode komunikatem `Command would have run if not in check mode` i przerywa play (`failed=1`), więc check mode nie jest w stanie orzec o idempotencji. Do zamknięcia potrzebne okno serwisowe na pełny converge obu klastrów.
