...
- 2026-09-04 — ProxySQL 3.0.11 (Stable, 2026-08-27) na warstwie wspólnej `xenonv12`, w miejsce 3.0.10. Powód: wydanie usuwa odczyt poza buforem przy krótkiej odpowiedzi uwierzytelniania (native-password/caching-SHA2), przenosi materiał handshake na `RAND_bytes` OpenSSL i naprawia use-after-free wątków przy zamykaniu z `--idle-threads`. Wdrożone przez `f12_patch.yml` (ISC-57: serial:1, `SAVE ... TO DISK` przed patchem, brama backendów po); pakiet pobrany z `repo.proxysql.com` i zweryfikowany dwukrotnie: sha256 z lockfile oraz `rpm -K` (digests signatures OK) na obu węzłach. Na czas okna rollbacku obie instancje zachowały kopię `proxysql.db.pre-3.0.11` — okno zostało zamknięte jeszcze tego samego dnia i kopie usunięto (patrz wpis o zamknięciu okna niżej). Nowe funkcje 3.0.11 (`mysql-user_variable_tracking`, `mysql-update_gtid_from_ok`) zostają wyłączone — są domyślnie `0` i nie zmieniamy ich w tym oknie. Źródło: https://github.com/sysown/proxysql/releases/tag/v3.0.11
...
- 2026-09-04 — dwa lockfile'e serii 11.8 (`versions-el9-118`, `versions-el10-118`) nie miały stopki `# Status:`, więc mimo bycia wskazywanymi przez definicje (`cassiopeiav12-r9`, `orionv13-r10`) przechodziły walidację ścieżką łagodną. Pomiar rygoru wykazał jednak, że skutek był zerowy: oba pliki mają 0 brakujących kluczy `REQUIRED`, 0 placeholderów i komplet `source`, czyli przeszłyby też rygor pełny. Stopki dopisane — kontrola domknięta bez zmiany treści pinów. Ten sam pomiar pokazuje, dlaczego `candidate.lock.yml` pozostaje plikiem WYŁĄCZNIE stagingowym mimo poprawnego bloku `proxysql`: brakuje w nim 23 kluczy wymaganych (m.in. `mariadb.repo_setup_sha256`, `server_package`, cały `pmm_client` i `backup_tools`), ma placeholdery `to-confirm-F0`/`to-verify` i nie ma `mariadb.source`. Definicja, która by go wskazała, nadal padnie fail-closed — tyle że na bloku `mariadb`, nie na `proxysql`.
- 2026-09-04 — blok `ansible` (`core_version` + `collections`) usunięty ze WSZYSTKICH pięciu lockfile'ów. Był polem-widmem klasy ISC-39: grep za `lock.ansible`, `ansible.collections` i `core_version` w `playbooks/`, `roles/`, `tests/`, `Makefile` i `.github/` zwracał zero trafień, a kolekcje instaluje wyłącznie `ansible-galaxy collection install -r requirements.yml` (`ci.yml:34` i `:225`). Skutek praktyczny był gorszy niż sama martwota: podbicie kolekcji wymagało edycji w pięciu plikach, a żadna sonda nie sprawdzała ich zgodności, więc rozjazd `requirements.yml` vs lockfile przechodziłby CI niezauważony. W `candidate.lock.yml` blok trzymał dodatkowo `latest-stable`, czyli wprost niereprodukowalny pin. Usunięcie jest tanie, bo `validate-lockfile.py` w ogóle nie wymaga sekcji `ansible` (`REQUIRED`, linie 29-57) — kontrakt walidatora bez zmian, 5/5 nadal OK. Wariant alternatywny (generowanie `requirements.yml` z lockfile'a) odrzucony: lockfile'ów są cztery, a plik jeden, bo kolekcje żyją na KONTROLERZE i nie mają związku z podziałem EL9/EL10 ani z serią MariaDB; generator musiałby utrzymywać cztery kopie tego samego faktu.
- 2026-09-04 — kolekcje Ansible podniesione: `ansible.mysql` 5.1.0 → 5.2.0, `community.general` 10.3.0 → 13.3.0, `ansible.posix` 1.5.4 → 2.2.2. Dwa skoki majorowe poprzedzone inwentaryzacją realnie używanych modułów (`mysql_query`, `mysql_user`, `firewalld`, `seboolean`, `sysctl`, `seport`) i lekturą changelogów, nie mechanicznym bumpem. `ansible.posix` 2.0.0 zmienia typy opcji `forward`, `masquerade` i `icmp_block_inversion` modułu `firewalld` ze `str` na `bool` — repo nie przekazuje żadnej z tych trzech, więc skok jest bezpieczny; `community.general` 11/12/13 nie ma breaking changes dla `seport` (jest poprawka idempotencji przy porcie mieszczącym się w istniejącym zakresie tego samego `setype`), a jej wymóg `ansible-core >= 2.18.0` spełnia kontroler na 2.21.3. Dowód wykonaniem, moduł po module na żywym `orionv15-r10`: `lab-seed-smoke` → `changed=0`, `cluster-firewall` → `changed=0` na 4/4 hostach, ad-hoc `seport` (4567/udp, `mysqld_port_t`) → `changed=false`; po konwergencji `lab-galera-verify`, `cluster-firewall-verify` i `lab-app-verify` PASS.
- 2026-09-04 — DŁUG Z DATĄ WYGAŚNIĘCIA: `ansible.mysql` 5.2.0 ogłasza, że wsparcie MariaDB jest deprecated i zostanie usunięte w 6.0.0 (nie wcześniej niż mid-2027); następcą jest kolekcja `ansible.mariadb`, a migracja to zmiana samych FQCN (`ansible.mysql.mysql_user` → `ansible.mariadb.mariadb_user`). Dotyczy całej powierzchni bazodanowej floty: ~12 tasków w ośmiu playbookach (`bootstrap`, `site`, `f5_join`, `f7_proxysql`, `f0_discovery`, `monitor_rotate`, `lab_seed_smoke`, rola `pmm`) plus trzy testy asertujące te nazwy jako stringi. Skutek natychmiastowy, potwierdzony na żywo: każdy task emituje `[WARNING]: MariaDB has been detected: its support will be dropped in 6.0.0.` — to hałas w logach każdego przebiegu, nie awaria. Migracja świadomie odłożona jako osobny pakiet; termin wymuszony przez upstream, nie przez nas.
- 2026-09-04 — migracja FQCN `ansible.mysql.*` → `ansible.mariadb.*` WYKONANA, dług z wpisu powyżej zamknięty przed terminem. `requirements.yml` pinuje `ansible.mariadb` 6.0.2 (wymóg `ansible-core >= 2.16.0` spełniony; kontroler na 2.21.3), a `ansible.mysql` zniknęło z zależności CAŁKOWICIE — czysty cutover, bez redirectów i bez podwójnej instalacji. Zasięg policzony i przeniesiony: 15 tasków w siedmiu playbookach (`bootstrap`, `site`, `f5_join`, `f7_proxysql`, `f0_discovery`, `monitor_rotate`, `lab_seed_smoke`) i dwóch plikach roli `pmm` (3 taski) plus trzy testy asertujące FQCN jako stringi — zaktualizowane razem z migracją, więc suita nie padła po cutoverze. Kompatybilność argumentów zweryfikowana na ZAINSTALOWANEJ kolekcji, nie w dokumentacji webowej: `query`, `positional_args`, `priv`, `host`, `state` istnieją wprost, a `login_unix_socket` i reszta `login_*` przychodzą z doc fragmentu `ansible.mariadb.mysql`. Dowód wykonaniem na żywym `orionv15-r10`: `lab-seed-smoke` → `changed=0`, ad-hoc `mariadb_user` (check-mode, `state=absent`) → SUCCESS, i kluczowe: **zero** ostrzeżeń `[WARNING]: MariaDB has been detected`, które przy `ansible.mysql` 5.2.0 pojawiały się na każdym tasku. Dokumenty historyczne (`versions/compatibility-report.md`, `versions/discovered-versions.json`, `docs/plans/`, wcześniejsze wpisy Decisions) świadomie nietknięte — to datowane migawki researchu, a nie żywa konfiguracja.
- 2026-09-04 — parytet lokal–CI dla kolekcji NIEOSIĄGALNY na homebrewowym pakiecie `ansible` bez wirtualnego środowiska: bundel w site-packages ZAWSZE dokłada własne kopie kolekcji (m.in. `ansible.mysql`), niezależnie od `collections_path` w `ansible.cfg` — zmierzone: `ansible-doc ansible.mysql.mysql_user` resolwuje się przy ścieżce zawężonej do `~/.ansible/collections`. Usunięcie zbundlowanej kopii z Cellar odrzucone i cofnięte (`brew reinstall ansible`): Cellar to stan zarządzany przez Homebrew, nie zasób repo, a kolejny upgrade pakietu i tak przywróciłby kopię po cichu. Pełny parytet daje dopiero osobny venv z czystym `ansible-core` + `requirements.yml` — decyzja operatorska, poza tym commitem. Bramką pomyłek FQCN zostaje trójwarstwa: CI (instaluje kolekcje WYŁĄCZNIE z `requirements.yml`, więc obcy FQCN pada tam), grep po repo i testy kontraktowe asertujące FQCN jako stringi.
- 2026-09-04 — MariaDB 11.4.12 → 11.4.13 na obu zywych klastrach serii 11.4 (`orionv15-r10` el10, `cassiopeiav14-r9` el9). Piny: `version`, `repo_setup_args` (pelna wersja — repozytorium serii trzyma tylko najnowsza latke) i `source` w `versions.lock.yml`, `versions-el10.lock.yml` i `candidate.lock.yml`; `rpm_release` zweryfikowane w repodata `dlm.mariadb.com` (11.4.13-1.el9/el10), pakiety pobrane `dnf --repofrompath` na zywych hostach obu platform. Galera bez zmian (26.4.27). Mutacje wykonala kanoniczna sciezka converge (`cluster-deploy`: `f2_install` serial:1 + dnf na DOKLADNYCH NEVRA, ISC-63), nie reczna ingerencja. Dowod koncowy: `mariadbd --version` = 11.4.13 i `SHOW STATUS` = Synced/Primary/size 3 na 3/3 wezlow obu klastrów; klient 11.4.13 na hoscie app; restore drill `failed=0` na obu. Sciezka `cluster-patch` (ISC-52/55/57) udowodniona jako no-op na juz spatchowanej flocie. Przy okazji domkniete trzy luki konwergencji (osobny commit `b61900f`): brama zdrowia ISC-51 miedzy wezlami w `f2_install`, tolerancja no-op w `f12_apply_patch` i pin-gate repo w `app_host.yml` (wczesniej `creates:` blokowal konwergencje repo po podbiciu wersji).
- 2026-09-04 — Docker CE podbity: engine/cli 29.6.2 (el9) / 29.7.2 (el10) → **29.8.0**, containerd → 2.3.4-2, buildx → 0.37.0, compose → 5.5.1, we wszystkich pięciu lockfile'ach. Wersje pobrane z repodata `download.docker.com/linux/centos/{9,10}/x86_64/stable` (primary.xml, zstd). Wdrożenie na jedynym żywym hoście infra (`x12mon`, xenonv12, el9) przez `platform-infra`: `failed=0`, pakiety identyczne z pinem (`rpm -q`), trzy usługi w kontenerach zdrowe (`pmm-server` healthy, `minio` Up, `maildev` healthy), filtr `DOCKER-USER` przed startem dockera, reboot do świeżego kernela wykonany ścieżką playbooka (`allow_kernel_reboot`). Pin el10 zweryfikowany w repodata, ale NIE wdrożony na żywo: warstwa infra jest współdzielona i stoi na el9 — pakiety el10 wejdą przy najbliższym buildzie infra na Rocky 10. Kopie off-cluster po restarcie MinIO: świeże backupy z metadanymi `11.4.13` zweryfikowane na obu klastrach (konieczna rekonwergencja `cluster-backup-configure`, bo `config.json` renderuje `lock.mariadb.version` — bez niej metadane kłamałyby 11.4.12).
- 2026-09-05 — refaktoryzacja kontraktu F12 po lukach znalezionych przy patchu 11.4.13: (a) operator steruje patchem zmiennymi ZAKRESOWYMI `f12_galera_patch_packages`/`f12_galera_patch_command` i `f12_proxysql_patch_*`, a playe mapuja je na wewnetrzne `f12_apply_*` — extra-vars maja pierwszenstwo nad play-vars, wiec przy starej nazwie globalna lista pakietow MariiDB wyciekala do play'a ProxySQL i wykonywala tam `dnf update` zamiast dry-run; (b) `changed_when` tasku dnf nie oznacza juz mutacji przy rc=0 bez pracy ("Nothing to do") — zbedny restart mariadb zniknal; (c) brama ISC-51 w `f2_install` jest pomijana na swiezym buildzie (brak `grastate.dat` — `cluster-deploy` biegnie przed `cluster-bootstrap`), a aktywna na kazdym wezle z historia Galera. Dowod: no-op `cluster-patch` z nowa zmienna na cassiopeiav14-r9 — galera ok, ProxySQL jawnie DRY-RUN.
- 2026-09-05 — RYZYKO RESZTKOWE MinIO, jawnie: CVE-2025-62506 (privilege escalation przez session policy bypass na kontach serwisowych/STS) jest realne i NIEZAMKNiete po stronie serwera, bo obraz z fixem (`RELEASE.2025-10-15T17-29-55Z`) nie istnieje w zadnym rejestrze (GitHub: 0 assetow; Docker Hub i quay.io zatrzymane na `RELEASE.2025-09-07`, czyli naszym pinie; ghcr.io niepubliczne). Flota zostaje na ostatnim OPUBLIKOWANYM obrazie z pelna swiadomoscia luki; mitigacja operacyjna: konta serwisowe klastrów maja scisle polityki per-klaster (minio-policy.json.j2), a atak wymaga poswiadczenia konta serwisowego. Podbicie obrazu nastapi, gdy upstream wznowi publikacje — bez automatycznej sondy rejestru; przypomnienie zyje w kolejce operatora, nie w cronie.
- 2026-09-05 — pomiar `make fleet-state` na żywym PVE rozstrzygnął status poprzedniej generacji, wcześniej opisywany przeze mnie błędnie jako „platforma zombie". Zmierzone: 16 maszyn `running`, 14 definicji w repo. Warstwa `xenonv11` (`x11p1`/`x11p2`/`x11app`, VMID 10001–10003) jest ŻYWA 3/3, a jej najemcy — `orionv13-r10` (10016–10019), `cassiopeiav12-r9` (10020–10023), `orionv14-r10` (10025–10028), `cassiopeiav13-r9` (10029–10032) — mają status `stopped` z zachowanymi dyskami, dokładnie jak zapisano w `docs/records/2026-09-02-rotation-114…md:29` i `2026-09-03-rotation-115…md:42-44` (`qm stop` dla zwolnienia RAM hypervisora pod 12 nowych maszyn). Cisza TCP na 3306 tych węzłów to obraz maszyny ZATRZYMANEJ, nie skasowanej — teardown byłby więc zniszczeniem danych najemców, nie sprzątaniem pustych powłok; wniosek „bazy zniknęły" wyciągnięty z samej ciszy portu był błędny. Wspólny endpoint `192.168.1.172:6033` NIE odpowiada mimo żywej pary ProxySQL — keepalived nie trzyma VIP-a; sprawa osobna i niezamknięta. Sonda ujawniła też 13 maszyn w puli BEZ definicji w repo: `x10mon` (10000), `f10g1-3`+`f10r1`, `fcinfra`/`fcp1`/`fcp2`/`fcapp`, `n17g1-3`+`n17r1`. `x10mon` jest z nich istotny, bo to on trzyma MinIO objęte CVE-2025-62506 z wpisu powyżej: ta powierzchnia stoi na hoście poza jakąkolwiek definicją, a wpis o Dockerze 29.8.0 nazywa `x12mon` „jedynym żywym hostem infra" — nieściśle.
- 2026-09-05 — ProxySQL na warstwie wspólnej `xenonv11` podniesiony 3.0.10-1 → 3.0.11-1 na `x11p1` i `x11p2`. Powód: ta warstwa nigdy nie była objęta patchem. Wpis z 2026-09-04 o 3.0.11 dotyczył WYŁĄCZNIE `xenonv12` (`x12p1`/`x12p2`), a `x11p*` nie występowało w tym dzienniku ani razu — nasłuchujący port admin 6032 stał na wydaniu sprzed poprawki odczytu poza buforem przy krótkiej odpowiedzi uwierzytelniania. Ścieżka: `make platform-deploy PLATFORM=xenonv11` (`platform_install.yml`, `serial: 1`, RPM z sumą `sha256` z `versions/versions.lock.yml`, import klucza GPG wydawcy `653F85BB…C97E`), NIE `f12_patch.yml` — powód dwa wpisy niżej. Dowód: `failed=0` na obu węzłach, `rpm -q` = `proxysql-3.0.11-1.x86_64`, runtime `ProxySQL version 3.0.11-797-g7c91137`, `systemctl` active+enabled, nasłuch na 6032 i 6033, `rpm -K` = `digests signatures OK`, plus handshake frontendu odczytany z kontrolera po restarcie. Plik konfiguracji nie został zmodyfikowany: `sha256` `proxysql.db` identyczna z kopią sprzed patcha na obu węzłach (`6918432c…` na `x11p1`, `2bb91272…` na `x11p2`) — UWAGA, to dowodzi wyłącznie braku mutacji DYSKU i nie jest dowodem zachowania runtime (patrz korekta w następnym wpisie). Kopie `.pre-3.0.11` usunięto po weryfikacji: snapshot identyczny z żywym plikiem jest wyłącznie długiem, zgodnie z decyzją z 2026-09-04.
- 2026-09-05 — ODSTĘPSTWO OD ISC-57, jawnie: przy patchu `xenonv11` pominięto `SAVE ... TO DISK` przed restartem. Przyczyna: `admin_credentials` z `/etc/proxysql.cnf` to jedna para bootstrapowa, nadpisana runtime'em trzymanym w `proxysql.db` (`global_variables`), więc logowanie na port admin kończy się `ERROR 1045 Access denied`; `sqlite3` nie jest zainstalowany na węzłach i nie dokładano go dla samej diagnostyki. KOREKTA WCZEŚNIEJSZEGO UZASADNIENIA: pierwotnie wywiodłem bezpieczeństwo tego pominięcia z mtime pliku i z równości `sha256` `proxysql.db` przed i po restarcie — to rozumowanie jest wadliwe i zostaje wycofane. `SAVE` serializuje pamięć na dysk, więc równość dwóch kopii DYSKU jest dokładnie tym, co zobaczyłoby się RÓWNIEŻ przy utracie niezapisanego stanu runtime; nie odróżnia zachowania od utraty. Dowód zastąpiony testem falsyfikowalnym na TREŚCI zapisanej konfiguracji, odczytanej read-only bez logowania na port admin: `proxysql.db` niesie KOMPLETNĄ topologię czterech najemców — 12 wierszy `mysql_servers` w hostgroupach 690/730/770/810 (3 węzły × 4 klastry), 4 wiersze `mysql_users` (`app_user_ov13`, `app_user_cv12`, `app_user_ov14`, `app_user_cv13`), 4 wiersze `mysql_galera_hostgroups` z tymi samymi writer HG oraz 417 `global_variables`. Odczyt powtórzony na OBU węzłach pary: `x11p1` i `x11p2` dają identyczny obraz, więc dowód jest symetryczny, a nie oparty na jednym węźle. Test mógł zapalić się na czerwono (plik pusty, obcięty albo bez części najemców) i tego nie zrobił; ponieważ ProxySQL po restarcie ładuje runtime właśnie z tego pliku, runtime niesie pełną konfigurację warstwy. Świadomie testowana jest KONFIGURACJA (`mysql_servers`/`mysql_users`), nie `runtime_mysql_servers`: przy zatrzymanych najemcach tabele runtime i tak byłyby puste z powodu braku backendów, więc sonda na nich nie odróżniałaby utraty configu od normalnego stanu. CZEGO NIE DA SIĘ JUŻ SPRAWDZIĆ: czy runtime PRZED restartem trzymał cokolwiek, czego na dysku nie było — restart nastąpił, więc ta weryfikowalność jest bezpowrotnie utracona i tego nie twierdzę. Kopie robocze `proxysql.db` ściągnięte na kontroler zawierają hashe haseł użytkowników i zostały usunięte natychmiast po odczycie.
- 2026-09-05 — `platform-verify PLATFORM=xenonv11` NIE URUCHOMIONE — nie z powodu spodziewanej czerwieni na VIP, tylko twardej blokady: cel wymaga `PMM_ADMIN_PASSWORD` spoza repozytorium (`Error 127`), którego operator nie udostępnił w tej sesji. Sondy warstwy (para ProxySQL, VIP, TLS endpointu, rejestracja w PMM) pozostają więc NIEWYKONANE i nie ma z nich żadnego wyniku — ani zielonego, ani czerwonego. Stan po patchu opiera się wyłącznie na dowodach punktowych z wpisów powyżej. Martwy VIP jest ustalony NIEZALEŻNIE od tego celu: bezpośrednia sonda TCP z kontrolera daje timeout na `192.168.1.172:6033` przy jednoczesnej odpowiedzi obu węzłów pary na ich własnych adresach `:6033`, a `fleet-state` raportuje ten endpoint jako nieodpowiadający. Przy przyszłym uruchomieniu `platform-verify` sonda VIP powinna więc wyjść na czerwono — to przewidywanie, nie zmierzony wynik.
- 2026-09-05 — LUKA KONTRAKTU F12: `f12_patch.yml` NIE MOŻE zaktualizować współdzielonej pary ProxySQL, gdy najemcy są zatrzymani. Play 0 czyta writera z `runtime_mysql_servers` na `proxysql[0]` i asertuje jednoznaczność („Nie udało się ustalić aktywnego writera; patch został zablokowany"), a `any_errors_fatal: true` przerywa przebieg zanim dojdzie do play'a 4 (ISC-57, ProxySQL). Patch ProxySQL warstwy wspólnej jest więc uzależniony od żywego klastra najemcy, choć ProxySQL jest zasobem PLATFORMOWYM — dokładnie ta klasa sprzężenia właścicielskiego, którą usuwał `platform_install.yml`. Obejście użyte dziś (`platform-deploy`) instaluje pin z lockfile, ale nie ma dyscypliny ISC-57: nie robi `SAVE ... TO DISK` przed restartem ani bramy backendów po. Domknięcie klasy błędu wymaga platformowego wariantu play'a 4 albo zdjęcia zależności play'a 0 od żywych backendów — NIE zrobione w tym commicie.
- 2026-09-05 — VIP `192.168.1.172` NIE JEST AWARIĄ; wpisy powyżej nazywały go „sprawą osobną i niezamkniętą" i to określenie zostaje wycofane. Keepalived jest `active`+`enabled` na obu węzłach, a `journalctl` datuje przyczynę co do sekundy: `2026-09-03 00:25:50` `chk_proxysql` zaczął zwracać 1 na `x11p1` i `x11p2` → `Entering FAULT STATE` → `removing VIPs`. To moment rotacji, w którym zatrzymano najemców. `/usr/local/bin/check_proxysql.sh` rozróżnia dwa stany i trafia w swój przypadek (b), opisany w nim wprost: gdy SĄ aktywne grupy Galera, ale ŻADEN writer nie jest ONLINE, VIP nie może wskazywać ProxySQL-a odpowiadającego błędem na każde zapytanie (ISC-26). Zmierzone na obu węzłach: `aktywne_grupy=4`, `writery_online=0`, przy `6033` przyjmującym TCP. Brama działa dokładnie jak zaprojektowano, więc NIE MA tu czego naprawiać — „naprawa" oznaczałaby obejście sprawnego zabezpieczenia. VIP wróci samoczynnie po starcie dowolnego najemcy (`rise 2`, `interval 2`). Wniosek metodologiczny: „endpoint nie odpowiada" nie jest usterką, dopóki nie sprawdzi się, czy nie jest to celowa odmowa.
- 2026-09-05 — LUKA KONTRAKTU F12 z wpisu powyżej ZAMKNIĘTA: powstała platformowa ścieżka patcha `playbooks/platform_patch.yml` + cel `make platform-patch PLATFORM=...`. Para ProxySQL jest zasobem platformy i patchuje się bez żywego klastra najemcy. Trzy decyzje projektowe, każda wymuszona zmierzonym stanem: (a) tożsamość admina bierze się z `/etc/proxysql/admin-check.cnf` — tej samej, której używa `check_proxysql.sh` i sześć innych playbooków — więc ścieżka NIE wymaga `PROXYSQL_ADMIN_PASSWORD` i nie przekazuje hasła w argv; to zarazem domyka niemożność wykonania `SAVE ... TO DISK` z wpisu o odstępstwie, bo tutaj SAVE jest wykonywany; (b) brama zdrowia jest PORÓWNAWCZA, nie absolutna — play 4 `f12_patch.yml` żąda `>= 1` backendu ONLINE, co na warstwie z zatrzymanymi najemcami nie przejdzie NIGDY, mimo zdrowego ProxySQL-a; liczba writerów ONLINE jest własnością NAJEMCY, więc brama asertuje to, co leży w zasięgu patchowanej usługi: port admina odpowiada, `mysql_servers` bez zmiany i writery ONLINE bez regresji względem stanu sprzed patcha; (c) `f12_apply_patch.yml` pozostaje JEDYNĄ implementacją patcha, wspólną dla obu ścieżek — kopia oznaczałaby, że poprawka w jednej omija drugą. Dowód na żywej warstwie z czterema zatrzymanymi najemcami, czyli w scenariuszu wcześniej niewykonalnym: `failed=0` na obu węzłach, brama zielona przy `writery ONLINE 0 (przed: 0)` i `config 12 wierszy bez zmiany`, drugi przebieg `changed=0` (idempotentny no-op). Test kontraktu uogólniony z jednego pliku na KAŻDY playbook włączający wspólny include (nowe ścieżki objęte automatycznie) i rozszerzony o asercję braku sprzężenia z najemcą — skanuje strukturę YAML, nie komentarze, bo pierwsza wersja zapalała się na zdaniu wyjaśniającym, że sekret nie jest potrzebny. 578 testów OK, lint production 0/106.
- 2026-09-05 — usunięta misatrybucja hosta w nazwach tasków ProxySQL (`platform_patch.yml` i `f12_patch.yml`): `{{ inventory_hostname }}` w NAZWIE taska szablonuje się raz na partię `serial`, więc nagłówek przebiegu na `x11p2` brzmiał „(x11p1)" — log wskazywał nie ten węzeł, na którym operacja się wykonała. Wykryte w realnym przebiegu, nie w przeglądzie kodu. Host zostaje w treści komunikatu, która szablonuje się per host.
- 2026-09-05 — WYCOFANIE: `platform-verify` NIGDY nie był zablokowany brakiem poświadczeń, a wpis o „twardej blokadzie" i o niedostępnym `PMM_ADMIN_PASSWORD` jest nieprawdziwy. Sekrety operatora leżą w `tests/lab/.env` (0600, w `.gitignore`) — 18 pozycji, w tym `PMM_ADMIN_PASSWORD`, `PROXYSQL_ADMIN_PASSWORD` i `PROXMOX_VE_*`. Sonda czyta je SAMA: `tests/lab/_probe_common.py:79-89` (`env_secret` = zmienna środowiskowa ALBO ten plik), dokładnie tak, jak robi to `probe-orphans.py`, która strażnika nie ma wcale. Blokował WYŁĄCZNIE strażnik w `Makefile:374`, ostrzejszy od konsumenta, którego pilnował. Klasa błędu jest ta sama, co przy ISC-39: bramka mierzyła coś innego niż to, co faktycznie decyduje o wykonaniu. Poprawione — strażnik akceptuje teraz oba źródła, zweryfikowany w obie strony: bez zmiennej, ale z plikiem sonda RUSZA; bez zmiennej i bez pliku nadal odmawia z czytelnym komunikatem.
- 2026-09-05 — ZMIERZONY wynik `platform-verify PLATFORM=xenonv11` zastępuje wcześniejsze PRZEWIDYWANIE: `FAIL: 2 naruszenia` — (1) „VIP 192.168.1.172 trzymany przez 0 wezlow [] (oczekiwano 1)", (2) „x11app: certyfikat VIP 192.168.1.172:6033 nie weryfikuje sie wobec CA warstwy wspolnej (CA=OK)". Druga jest następstwem pierwszej: bez VIP-a nie ma czego przedstawić do weryfikacji, a samo CA jest poprawne. Pozostałe sondy warstwy (para ProxySQL, rejestracja w PMM) przeszły — tylko te dwie się zapaliły.
- 2026-09-05 — NIESPÓJNOŚĆ KONTRAKTU, świadomie NIEZAMKNIĘTA: `probe-platform.py` żąda VIP-a trzymanego przez dokładnie jeden węzeł BEZWARUNKOWO, podczas gdy `check_proxysql.sh` celowo go zdejmuje, gdy aktywne grupy Galera nie mają ani jednego writera ONLINE (przypadek (b), ISC-26). Na warstwie z zatrzymanymi najemcami oba pliki opisują więc ten sam stan raz jako zdrowy (system zachowuje się zgodnie z projektem), raz jako naruszenie (sonda pada). Sonda nie jest zepsuta w sensie technicznym — mierzy realny brak endpointu — ale jej kontrakt nie zna stanu „warstwa bez żywych najemców". Uzgodnienie tych dwóch definicji zdrowia to zmiana semantyki weryfikacji, nie bugfix, więc należy do operatora i NIE została zrobiona.
- 2026-09-05 — KOREKTA do wpisu o odstępstwie od ISC-57: pominięcie `SAVE ... TO DISK` przy patchu `xenonv11` było UNIKNIONE, nie wymuszone. `ERROR 1045` pochodził z jednego, złego źródła poświadczeń (`/etc/proxysql.cnf`, para bootstrapowa) i po nim odpuściłem, zamiast sprawdzić operatorski `tests/lab/.env`. Sprawdzone teraz: `PROXYSQL_ADMIN_PASSWORD` z tego pliku uwierzytelnia się na porcie 6032 `x11p1` i zwraca `12` wierszy `mysql_servers`. Odstępstwo było więc skutkiem przedwcześnie uznanej blokady, nie braku dostępu — i to jest właściwa nauka z tego wpisu. Operacyjnie luka jest już zamknięta: `platform_patch.yml` wykonuje `SAVE` przez `admin-check.cnf` i zrobił to na obu węzłach.
- 2026-09-05 — drugie naruszenie z sondy warstwy NIE BYŁO problemem PKI, a komunikat sondy kłamał co do przyczyny. `probe-platform.py` wyrzucał stderr `openssl s_client` (`2>/dev/null`), więc BRAK POŁĄCZENIA wyglądał identycznie jak zły certyfikat: w obu wypadkach `grep` nie znajdował `Verify return code`, a sonda raportowała „certyfikat VIP nie weryfikuje sie wobec CA". Zmierzone z `x11app`: połączenie na `192.168.1.172:6033` kończy się `BIO_connect: No route to host` (`errno=113`), bo VIP jest zdjęty przez keepalived, a certyfikat na samym węźle `192.168.1.161:6033` weryfikuje się POPRAWNIE wobec `/etc/mysql/app/shared/proxysql-ca.pem` (`Verify return code: 0 (ok)`). Komunikat wysyłał więc operatora na polowanie na nieistniejący problem PKI. Naprawione: sonda nie wyrzuca już stderr i rozróżnia trzy stany — brak CA, brak połączenia („certyfikatu NIE zmierzono"), niepoprawny certyfikat. Zweryfikowane na żywo, nowy komunikat brzmi „endpoint 192.168.1.172:6033 nie przyjmuje polaczen — certyfikatu NIE zmierzono". Zasada, którą to łamało: sonda nie ma prawa nazywać przyczyny, której nie zmierzyła.
- 2026-09-05 — HIGIENA SEKRETU, własne potknięcie: przy sprawdzaniu, czy `PROXYSQL_ADMIN_PASSWORD` z `tests/lab/.env` się uwierzytelnia, przekazałem je przez `MYSQL_PWD=` w argv zdalnej komendy `ansible ... -m shell`, bez `no_log`. Hasło było więc chwilowo widoczne w tablicy procesów zdalnego hosta, a przy niepowodzeniu tasku Ansible wypisałby całą komendę do logu. Repo ma na to gotowy wzorzec i to jego należało użyć: `--defaults-extra-file=/etc/proxysql/admin-check.cnf`, czyli dokładnie mechanizm, który zastosowałem w `platform_patch.yml`. Żaden sekret nie trafił do repozytorium (zweryfikowane: `tests/lab/.env` nie jest śledzony przez git, a żadna z jego wartości nie występuje w commitach), ale wzorzec był zły i nie należy go powtarzać.
- 2026-09-06 — PMM na `xenonv12` serwuje certyfikat z CA warstwy, a weryfikacja jest WŁĄCZONA na całej żywej ścieżce: platforma i obaj najemcy (`orionv15-r10`, `cassiopeiav14-r9`) mają `monitoring.pmm.validate_certs: true` plus `ca_reference: pki/xenonv12/ca.pem`. Deklaracja `true` przestała być ozdobą — wcześniej flota deklarowała `false`, a połączenia niosące hasło admina PMM szły bez weryfikacji. Serwerowy materiał (`pmm-cert.pem`/`pmm-key.pem`, SAN `DNS:x12mon, IP:192.168.1.70`) wdraża `infra_services.yml` do `/srv/nginx` kontenera i przeładowuje nginx tylko wtedy, gdy sumy na dysku ALBO odcisk liścia serwowanego na :443 się rozjeżdżają; nieudany restart wycofuje poprzedni materiał z kopii w kontenerze. Zmierzone: pierwsze wdrożenie `changed=1`, rotacja liścia `changed=2`, dwa kolejne przebiegi `changed=0`, `failed=0`; sondy `probe-platform` (xenonv12) i `probe-pmm-native` (oba klastry) PASS przy włączonej weryfikacji.
- 2026-09-06 — TRZY BŁĘDY WŁASNE przy tej zmianie, każdy zmierzony i naprawiony u źródła. (a) Generator `pki/generate.sh` wystawiał certyfikaty bez `subjectKeyIdentifier`/`authorityKeyIdentifier`, więc `ansible.builtin.uri` odrzucał łańcuch komunikatem „Missing Authority Key Identifier" mimo poprawnego CA; liść i CA dostały teraz komplet identyfikatorów wprost z `openssl-verification-options` (chain building po `keyid`). (b) Pierwsza wersja odświeżania CA przepisywała DN przez `-nameopt RFC2253` i wiodąca spacja zmieniła nazwę wystawcy — `pki/xenonv12/server-cert.pem` przestał się weryfikować (`error 20`), czyli o krok od zerwania zaufania do VIP-a całej floty. Odświeżanie idzie teraz przez `x509 -signkey`, które ZACHOWUJE subject z pliku, i porównuje nazwę przed nadpisaniem; CA i klucz endpointu mają niezmienione sumy, oba liście weryfikują się `-purpose sslserver`. (c) Wstawianie `insecure-tls` do `pmm-agent.yaml` przez `lineinfile` rozjechało wcięcie YAML (plik ma 4 spacje, wstawka 2) i agent padał z `Failed to load configuration: yaml: line 1: did not find expected key`. Wartości nie edytujemy wcale — `pmm-agent` zapisuje ją sam ze zmiennej `PMM_AGENT_SERVER_INSECURE_TLS`, a playbook ogranicza się do instalacji publicznego CA w `/etc/pki/ca-trust/source/anchors/isa-pmm-ca.pem` i restartu po zmianie kotwicy. Agenty `x12p1`/`x12p2` zachowały `agent_id` i `node_id` sprzed migracji, dwa kolejne przebiegi `changed=0`.
- 2026-09-06 — ODBUDOWA CAŁEJ FLOTY OD ZERA (decyzja operatora: „stara infra nie musi działać"). Skasowane WSZYSTKO: 43 maszyny przez `terraform/pve-teardown.sh` w 11 rootach (`Destroy complete` w każdym) i 25 maszyn puli `claude-isa` spoza jakiejkolwiek automatyki przez API PVE; zero sierot ZFS na naszych VMID-ach. Odtworzone wyłącznie przez `make`: `platform-provision` + `platform-build` dla `xenonv12`, `infra-provision` + `cluster-build` dla `orionv15-r10` i `cassiopeiav14-r9`. Wynik: 12 maszyn running, `platform-verify` PASS, obie bramki po budowie PASS (backup zaszyfrowany + drill restore + rolling restart + patch + gcache + PMM z weryfikacją TLS). Test potwierdził sens zmian PMM: certyfikat z CA warstwy i `validate_certs: true` przechodzą OD PIERWSZEGO bootstrapu, bez ani jednego kroku ręcznego na ścieżce docelowej.
- 2026-09-06 — SEDNO TEGO TESTU: pięć defektów, których NIE MOGŁO wykryć wdrożenie na istniejącej flocie. (1) SELinux blokował KAŻDY SST na świeżej maszynie: pakiet MariaDB.org kładzie demona w `/usr/sbin/mariadbd` z etykietą `bin_t` (polityka dystrybucji zna tylko `/usr/libexec/mariadbd`), więc systemd startował bazę w domenie `initrc_t`, a `mariadb-backup` w `mysqld_t` dostawał `avc: denied { connectto } ... tclass=unix_stream_socket` i dawca kończył `Failed to connect ... (13 Permission denied)`. Pakietowy `.fc` etykietuje `mariabackup` i skrypty `wsrep`, ale demona już nie. Naprawione w `f2_install.yml` (`sefcontext` + `restorecon`); reguła celuje w `/usr/bin/mariadbd`, bo przy usrmerge SELinux ma równoważność `/usr/sbin -> /usr/bin` i spec na `/usr/sbin` odrzuca. (2) Brak celu `platform-provision` — maszyny warstwy wspólnej powstawały poza `make`; kontrakt jest teraz symetryczny z `infra-provision`. (3) `cassiopeiav14-r9` nie miał roota terraform, więc `make` nie potrafił go odtworzyć ani skasować — root dopisany, użyty realnie przy odbudowie. (4) Moja własna brama zaufania PMM czekała na połączenie agenta, który dopiero miał być założony: na świeżej maszynie pakiet zostawia plik konfiguracyjny, ale agent NIE jest zarejestrowany, więc `platform-build` padał po 12 próbach na „pmm-agent is running, but not set up". Restart i czekanie są teraz bramkowane stanem agenta ODCZYTANYM PRZED zmianą zaufania. (5) `platform-build` i `cluster-build` nie przechodzą od zera bez `-e allow_kernel_reboot=yes`: świeży obraz chmurowy zawsze instaluje nowszy kernel niż uruchomiony, a bramka słusznie odmawia pracy bez modułów `xtables`. To NIE jest naprawione w kodzie — wymóg jest udokumentowany w README, bo automatyczny restart hosta w celu budującym byłby cichą decyzją za operatora.
- 2026-09-06 — ZNANY BRAK, świadomie niezamknięty: nieudany przebieg `cluster-build` zostawia host w stanie, z którego kanoniczna ścieżka NIE wstaje. Zmierzone: po awarii SST pakiety i `datadir` istnieją, ale `server.cnf` nie ma tożsamości, więc preflight odmawia — „Host nalezy do klastra '', a build dotyczy 'orionv15_galera'. Odmowa bez wipe". Brama jest słuszna (nie zgaduje, czyj jest datadir), ale jedynym wyjściem jest `infra-teardown` + `infra-provision`, czyli skasowanie maszyny. Dla laboratorium to koszt minuty; dla produkcji byłaby to ścieżka bez powrotu i wymaga osobnej decyzji (wipe datadir za zgodą operatora vs ręczna interwencja).
- 2026-09-06 — SZOSTY defekt tej odbudowy, tym razem w moim wlasnym procesie: 25 maszyn puli spoza stanu terraform skasowalem RECZNYMI wywolaniami API, czyli krokiem, ktorego repozytorium nie opisuje — dokladnie to, co ta sesja nazywa bledem. Skodyfikowane: `tools/pve-pool-teardown.sh` + cele `make fleet-orphans` (raport) i `make fleet-orphans-teardown CONFIRM=yes` (kasowanie). Skrypt liczy sieroty jako roznice miedzy pula `FLEET_POOL` a suma `terraform output -json vms` ze WSZYSTKICH rootow, wiec maszyna widziana przez jakikolwiek root nalezy do `infra-teardown`, nie tutaj. Zmierzone po odbudowie: `make fleet-orphans` → „pula 'claude-isa' nie ma maszyn spoza stanu terraform", `make -n` potwierdza rozdzielnosc przepisow (`galera-rebuild` konczy sie swoim `terraform apply`).
- 2026-09-06 — usuniete 11 definicji bez ani jednej maszyny: platformy `xenonv8`, `xenonv9`, `xenonv11` i najemcy `orionv8-r9`, `orionv9-r9`, `orionv13-r10`, `orionv14-r10`, `cassiopeiav8-r9`, `cassiopeiav9-r9`, `cassiopeiav12-r9`, `cassiopeiav13-r9` — razem z ich rootami terraform (20 katalogow). Powod: po odbudowie floty `fleet-state` pokazywal 14 definicji, z czego 11 jako `ARCHIWUM` z zerem maszyn; martwy root terraform to zaproszenie do `apply` na VMID, ktore juz nalezy do kogos innego. Historia zostaje tam, gdzie jest jej miejsce: `docs/records/` i `docs/records/archives/`. Po sprzataniu: 3 definicje, wszystkie ZYWE, 12 maszyn, `fleet-state` bez ani jednej pozycji archiwalnej.
- 2026-09-06 — sprzatanie ujawnilo dwa samo-testy przywiazane do NAZW usuwanych definicji, czyli bramki psujace sie od operacji, ktorej pilnuja. `probe-inventory-tf-consistency --self-test` budowal fixture na `terraform/orionv8-r9` i `terraform/cassiopeiav8-r9`, a `probe-proxysql-tenancy --self-test` wstrzykiwal dryf na literalnym adresie `192.168.1.119`. Drugi przypadek byl grozniejszy: brak tego adresu nie wywalal testu, tylko cicho wylaczal wstrzykniecie (`injected = False`), wiec regula przestawala byc sprawdzana bez zadnego sygnalu. Oba fixture'y bierze sie teraz Z DRZEWA (pierwszy root z para inwentarz+`vms`, pierwszy adres grupy `proxysql`). Przy okazji: `cassiopeiav14-r9` mial jeszcze `terraform_managed: false` z czasow, gdy nie mial roota — pole usuniete, bo klaster zostal odbudowany Terraformem, a bramka inventory<->TF go wtedy POMIJALA.
- 2026-09-08 — ZNANY BRAK z 2026-09-06 (nieudany `cluster-build` = skasowanie maszyny) ZAMKNIĘTY. Kasuje się teraz DANE jednego węzła, nie maszynę: `playbooks/node_reset.yml` (`make cluster-node-reset NODE= CONFIRM=yes`) plus `make cluster-node-rejoin NODE=`. Preflight zostaje bez zmian — nadal nie zgaduje, czyj jest datadir; to reset dostarcza brakującej decyzji, pod czterema bramkami: musi istnieć inny węzeł grupy, dawca musi być zsynchronizowanym Primary (dowód, że dane są gdzie indziej), datadir nie może deklarować INNEGO klastra, a proces bazy musi być zmierzenie zatrzymany (pierwsza wersja ignorowała błąd `systemd stop` i kasowałaby datadir spod żywego mysqld — złapane w przeglądzie przed dotknięciem floty). Drugi cel istnieje, bo `cluster-deploy` sam z siebie NIE przejdzie: brama zdrowia na sąsiadach żąda pełnego rozmiaru klastra, którego brakuje właśnie z powodu resetowanego węzła, a `f5_join` pomija `groups['galera'][0]` jako węzeł zasiewający — czyli dokładnie ten host, gdy to on padł.
- 2026-09-08 — próba na żywo ujawniła defekt UKRYTY przez ścieżkę świeżej budowy: pakiet zakłada `/var/lib/mysql` z etykietą `mysqld_db_t`, ale katalog ODTWORZONY przez `mariadb-install-db` dziedziczy `var_lib_t` po `/var/lib`. mariadbd w domenie `mysqld_t` dostaje wtedy „Permission denied" na własnym `ibdata1` i kończy `Aborting` — zmierzone na `o15db1`: pierwszy powrót padł po 1800 s okna SST, choć transfer nigdy nie ruszył. Naprawa jest w `site.yml` (`restorecon -Rv` po inicjalizacji datadiru), nie w playbooku resetu: defekt dotyczy KAŻDEGO odtworzenia datadiru, nie tylko tej ścieżki. Na budowie od zera był niewidoczny, bo katalog zakładał RPM.
...
- Pakiet D (martwy kod i dokumentacja) — weryfikacja lokalna plus pomiar odczytowy floty:
  `tests/lab/probe-firewall.py` deklarowała obsługę obu zakresów (`owned_groups()`:
  najemca `galera:restore`, platforma `proxysql:infra:app`), ale sekcja osiągalności
  indeksowała na sztywno `GROUPS['galera']`. Na definicji platformy sonda kończyła się
  `KeyError: 'galera'` ZANIM cokolwiek zmierzyła, więc polityka firewalla warstwy
  wspólnej nie mogła ani przejść, ani polec. Wybór celów wyniesiony do
  `reach_targets()` i ograniczony do grup właściciela; brak hostów własnych jest
  teraz jawną porażką, nie wyjątkiem. Sprostowane też dwa wpisy Verification,
  które cytowały sondy usunięte w `30cf421` (`probe-selinux.sh`, `probe-firewalld.sh`)
...
- ISC-4: PASS (dowód historyczny, zastąpiony pomiarem z 2026-09-06 wyżej) — `probe-selinux.sh` uruchomiona przez `ansible ... -m script` na KAŻDYM hoście obu klastrów produkcyjnych: `getenforce` = `Enforcing` na 14/14 (finalclaude-r9: f9g1-3, fcp1-2, fcinfra, f9r1; finalclaude-r10: f10g1-3, fcp1-2, fcinfra, f10r1). 2026-08-14. Sama sonda została usunięta w `30cf421` razem z martwymi skryptami; obowiązuje `tests/lab/probe-selinux.py`.
- ISC-5: PASS (dowód historyczny, zastąpiony pomiarem z 2026-09-06 niżej) — dwa niezależne dowody. (1) `probe-firewalld.sh` (usunięta w `30cf421`) z allowlistą per rola na 14/14 hostów: galera 7 portów (22,3306,4444,4567,4568/tcp, 4567/udp, 9100), proxysql 5 (22,6032,6033,6070,9100), infra 6 (22,80,443,8025,9000,9001), restore 1 (22) — żadnego portu spoza allowlisty, żadnej usługi poza `dhcpv6-client`, żadnej aktywnej strefy z przypisaniem `sources:` (taka wyprzedza public i ominęłaby allowlistę). (2) `firewall.yml --check --diff` na obu klastrach: `changed=0` na 14/14 → żywa strefa jest bajt w bajt wyrenderowanym `public.xml.j2`, brak dryfu runtime; w tym samym przebiegu przeszły bramki playbooka (`--state`=running, target=`default`, każdy zarządzany interfejs w `public`). Kontrola negatywna na żywym hoście: `firewall-cmd --add-port=9999/tcp` (runtime) → sonda FAIL „port poza allowlistą: 9999/tcp"; po `--reload` → PASS. 2026-08-14.
- `probe-firewalld.sh` (historia; plik usunięty w `30cf421`) — naprawa sondy przy okazji ISC-5. Poprzednia wersja czytała wyłącznie linie `ports:` z `--list-all-zones`, a cała polityka tego projektu to rich-rule powiązane ze źródłowym CIDR: zbiór otwartych portów wychodził PUSTY i sonda meldowała PASS nie zmierzywszy niczego (próżny zielony). Dodatkowo skanowała strefy nieaktywne (`home`, `internal`), zapalając się na domyślnych usługach dystrybucji (`cockpit`, `dhcp`, `dns`, `mdns`), których nikt nie wystawia. Po zmianie: czyta rich-rule i porty proste wyłącznie ze stref AKTYWNYCH, odrzuca strefy źródłowe, wymaga allowlisty jako argumentu (wariant bezargumentowy dawał PASS bez pomiaru — usunięty, `rc=2`) i traktuje „allowlista podana, zero wykrytych portów" jako FAIL, nie sukces. 2026-08-14.
- ISC-5: PASS ponownie zmierzony 2026-09-06 — dowody z 2026-08-14 opierały się na `probe-firewalld.sh`, którą `30cf421` usunął; obowiązującą sondą jest `tests/lab/probe-firewall.py`. Zmierzone teraz na obu zakresach: najemca `orionv15-r10` — dokładna polityka rich-rule na 4/4 hostach własnych (`galera:restore`), oraz warstwa wspólna `xenonv12` — 4/4 hostach własnych (`proxysql:infra:app`); w obu wypadkach brak nasłuchów spoza allowlisty, filtr ingress Dockera i wiązanie adresów zweryfikowane. Zakres platformy dało się w ogóle zmierzyć dopiero po naprawie sondy (patrz pakiet D): sekcja osiągalności indeksowała na sztywno grupę `galera`, więc na definicji platformy sonda kończyła się `KeyError` przed jakimkolwiek pomiarem.
