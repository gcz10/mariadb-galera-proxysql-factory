...
# INTERPRETER MA ZNACZENIE: `python3` w tych celach to systemowy /usr/bin/python3,
# ktory na macOS ma uprawnienie do sieci lokalnej. Ta sama sonda odpalona recznie
# interpreterem z venv/Homebrew dostaje EHOSTUNREACH na kazdy adres LAN i konczy
# sie UNDETERMINED (exit 2) — celowo, zeby nie ogłaszać porazki polityki floty,
...
cluster-firewall:  ## Wymuś minimalną politykę firewalld według roli hosta
	$(cluster_guard)
	ansible-playbook playbooks/firewall.yml $(CLUSTER_RUN) -e firewall_target_hosts=galera:restore $(ANSIBLE_OPTS)
# Jak wyzej: uruchamiaj przez ten cel (systemowy python3), nie recznie
# interpreterem z venv — inaczej sonda nie ma jak dosiegnac sieci laboratorium.
...
# 4096M mimo DWOCH pomiarow tej generacji — 4273M podczas budowy (2489129 B/s)
# i 4546M w pelnej bramie (2647889 B/s, maksimum z 5 rund). Obowiazuje wyzszy.
# Sonda nadal fail-closed; zaakceptowane ryzyko NIE zmienia FAIL w PASS, wiec
# ta bramka na v18 KONCZY SIE NIEZEREM i banner ponizej nie zostaje wypisany.
# Stara v17 jest zarchiwizowana (docs/records/archives/).
	$(TARGET_ENV) tests/lab/probe-gcache.py
	@echo "PASS: brama po budowie — wszystkie sondy stanu ustalonego zmierzone i zielone (zaakceptowane wyjatki ISC-68 musza byc wypisane przy klastrze w ISA.md)"
